...
if config.model == "iamm":
    from src.utils.decode.iamm import Translator
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, mask, topk=False):
        ...
        # Combine with values to get context
        contexts = torch.matmul(weights, values)  # torch.Size([1, 2, 40, 20])
...
def get_input_from_batch(batch):
    ...
    batch_size, sent_num, max_enc_len = enc_batch.size()
    assert len(enc_lens) == batch_size

    extra_zeros = None
    enc_batch_extend_vocab = None
    situation_ext_batch = None

    if config.pointer_gen:  
        enc_batch_extend_vocab = batch["input_ext_batch"]
        ext_shape = enc_batch_extend_vocab.shape
        enc_batch_extend_vocab = enc_batch_extend_vocab.reshape(ext_shape[0], ext_shape[1] * ext_shape[2]) 
        situation_ext_batch = batch["situation_ext_batch"]
...
def evaluate(model, data, ty="valid", max_dec_step=30):
    ...
    for j, batch in pbar:
        ...
        acc.append(acc_prog)
        if ty == "test":
            sents_g = model.decoder_greedy_batch(batch, max_dec_step=max_dec_step)  
            if config.model != "empdg":
                sent_b = ""  
            for i, greedy_sent in enumerate(sents_g):
                rf = " ".join(batch["target_txt"][i])  
                hyp_g.append(greedy_sent[0])  
                ref.append(rf)  
                ...
                temp = print_custum(
                    ...
                    ref=rf,
                    hyp_b= "",
                    hyp_g=greedy_sent[0],  # testBatch
                    pred_emotions=", ".join(top_preds[i]),  
                    comet_res=comet_res[i],  
                )
                results.append(temp)
        pbar.set_description(
            "loss:{:.4f} ppl:{:.1f}  acc:{:.4f}".format(np.mean(l), math.exp(np.mean(l)), np.mean(acc))
            # "loss:{:.4f} ppl:{:.1f}".format(np.mean(l), np.mean(p))
        )
    ...
    return loss, math.exp(loss), bce, acc, results  # modify testBatch 2023-4-15
    # return loss, ppl, bce, acc, results
