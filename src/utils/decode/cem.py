...
class Translator(object):
    ...
    def beam_search(self, src_seq, max_dec_step):
        ...
        with torch.no_grad():
            ...
            src_enc = self.model.encoder(src_emb, mask_src)
            ...
            for r in self.model.rels:
                ...
                if r != "x_react":
                    enc_output = self.model.cog_encoder(emb, mask)
                else:
                    enc_output = self.model.emo_encoder(emb, mask)
            ...
            dim = [-1, src_enc.shape[1], -1]
            if not config.woEMO:
                # Emotion
                emo_concat = torch.cat([src_enc, emo_cls.expand(dim)], dim=-1)
                emo_ref_ctx = self.model.emo_ref_encoder(emo_concat, mask_src)

            # Cognition
            cog_outputs = []
            for cls in cog_cls:
                cog_concat = torch.cat([src_enc, cls.expand(dim)], dim=-1)
                cog_concat_enc = self.model.cog_ref_encoder(cog_concat, mask_src)
            ...
            if config.woCOG:
                ...
                cog_ref_ctx = self.model.cog_lin(cog_ref_ctx)
